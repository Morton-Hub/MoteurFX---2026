...
export class ShapeMask {
  ...
  /** Masque vide couvrant un rectangle donné, borné au cadre. */
  static forRect(x0: number, y0: number, x1: number, y1: number, clipW: number, clipH: number): ShapeMask {
    const ax = Math.max(0, Math.floor(x0));
    const ay = Math.max(0, Math.floor(y0));
    const bx = Math.min(clipW - 1, Math.ceil(x1));
    const by = Math.min(clipH - 1, Math.ceil(y1));
    return new ShapeMask(ax, ay, Math.max(0, bx - ax + 1), Math.max(0, by - ay + 1));
  }

  /** Masque de la taille complète du cadre. */
  static full(clipW: number, clipH: number): ShapeMask {
    return new ShapeMask(0, 0, clipW, clipH);
  }

  count(): number {
    let n = 0;
    for (let i = 0; i < this.bits.length; i++) if (this.bits[i] === 1) n++;
    return n;
  }
  ...
  bounds(): { x0: number; y0: number; x1: number; y1: number } | null {
    ...
    return x1 < 0 ? null : { x0: x0 + this.x0, y0: y0 + this.y0, x1: x1 + this.x0, y1: y1 + this.y0 };
  }

  forEach(visit: (x: number, y: number) => void): void {
    for (let y = 0; y < this.h; y++) {
      for (let x = 0; x < this.w; x++) {
        if (this.bits[y * this.w + x] === 1) visit(x + this.x0, y + this.y0);
      }
    }
  }

  /** Ajoute les pixels d'un autre masque. */
  add(other: ShapeMask): void {
    other.forEach((x, y) => this.set(x, y));
  }

  /** Ne garde que les pixels présents dans les deux masques. */
  intersectWith(other: ShapeMask): void {
    for (let y = 0; y < this.h; y++) {
      for (let x = 0; x < this.w; x++) {
        if (this.bits[y * this.w + x] === 1 && !other.get(x + this.x0, y + this.y0)) {
          this.bits[y * this.w + x] = 0;
  ...
  /** Épaississement de `r` pixels (voisinage 4 répété). */
  dilate(r = 1): ShapeMask {
    let src: ShapeMask = this;
    for (let step = 0; step < r; step++) {
      const out = new ShapeMask(src.x0 - 1, src.y0 - 1, src.w + 2, src.h + 2);
      src.forEach((x, y) => {
        out.set(x, y);
        out.set(x - 1, y);
        out.set(x + 1, y);
  ...
  /** Amincissement de `r` pixels. Utile pour poser un liseré intérieur. */
  erode(r = 1): ShapeMask {
    let src: ShapeMask = this;
    for (let step = 0; step < r; step++) {
      const out = new ShapeMask(src.x0, src.y0, src.w, src.h);
      for (let y = 0; y < src.h; y++) {
        for (let x = 0; x < src.w; x++) {
          if (src.bits[y * src.w + x] !== 1) continue;
          const gx = x + src.x0;
          const gy = y + src.y0;
          if (src.get(gx - 1, gy) && src.get(gx + 1, gy) && src.get(gx, gy - 1) && src.get(gx, gy + 1)) {
            out.set(gx, gy);
          }
        }
      }
  ...
  /**
   * Distance de chaque pixel plein au vide le plus proche (chamfer 3-4,
   * normalisée en pixels). C'est cette épaisseur qui porte la valeur d'un
   * corps émissif : cœur clair, bord saturé.
   */
  innerDistance(): Float32Array {
    const n = this.w * this.h;
    const d = new Float32Array(n);
    const BIG = 1e9;
    for (let i = 0; i < n; i++) d[i] = this.bits[i] === 1 ? BIG : 0;
    const at = (x: number, y: number): number =>
      x < 0 || y < 0 || x >= this.w || y >= this.h ? 0 : (d[y * this.w + x] as number);
    for (let y = 0; y < this.h; y++) {
      for (let x = 0; x < this.w; x++) {
        const i = y * this.w + x;
        if (d[i] === 0) continue;
        let m = d[i] as number;
        m = Math.min(m, at(x - 1, y) + 3, at(x, y - 1) + 3, at(x - 1, y - 1) + 4, at(x + 1, y - 1) + 4);
        d[i] = m;
      }
    }
    ...
    for (let i = 0; i < n; i++) d[i] = (d[i] as number) / 3;
    return d;
  }

  /** Distance interne au pixel donné, en coordonnées écran. */
  distanceAt(dist: Float32Array, x: number, y: number): number {
    const lx = x - this.x0;
    const ly = y - this.y0;
    if (lx < 0 || ly < 0 || lx >= this.w || ly >= this.h) return 0;
    return dist[ly * this.w + lx] ?? 0;
  }

  /** Copie décalée. Les pixels dessinés restent intacts (décalage entier). */
  translated(dx: number, dy: number): ShapeMask {
    const out = new ShapeMask(this.x0 + Math.round(dx), this.y0 + Math.round(dy), this.w, this.h);
