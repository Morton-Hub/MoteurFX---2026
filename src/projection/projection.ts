 ...
 */
export function cameraPlane(p: ProjectionProfile): { right: Vec3; up: Vec3 } {
  // `right` est mis à l'échelle pour qu'une unité horizontale couvre le même
  // nombre de pixels qu'une unité verticale. Sans cette correction, une masse
  // construite ronde dans le plan sort ovale à l'écran — c'est ce qui donnait
  // des lobes en forme d'ailes au lieu de pétales.
  const k = Math.SQRT1_2;
  const horizontalPx = p.groundScale * 2 * k;
  const scale = horizontalPx === 0 ? 1 : p.heightScale / horizontalPx;
  return { right: { x: k * scale, y: -k * scale, z: 0 }, up: { x: 0, y: 0, z: 1 } };
}

/** Plan face caméra sans correction d'anisotropie, en unités monde strictes. */
export function cameraPlaneWorld(_p: ProjectionProfile): { right: Vec3; up: Vec3 } {
  const k = Math.SQRT1_2;
